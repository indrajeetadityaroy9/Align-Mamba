...
where αt ∈ (0, 1) is a data-dependent scalar-valued decay term that varies with t. Define the cumulative decay product γj = [�][j] i=1 [α][i][, and by expanding the recurrence, we can express the result in]
...
observations:Decay hurts memory retention. In the simplest S-NIAH-1 setting with repeated synthetic context, models memorize minimal information, testing long-term retention. DeltaNet achieves nearperfect performance across all sequence lengths. Mamba2 degrades significantly beyond 2K sequences since it decays historical information too quickly, while Gated DeltaNet’s degradation is
...
Table 4 presents results on real-world Table 4: Accuracy on recall-world retrieval tasks with input
recall-intensive tasks used by Arora et al. truncated to 2K tokens. SQD: SQUADE. TQA: Trivial QA.
(2024b). As expected, linear recurrent models show a significant performance gap compared to
Transformers, while hybrid models combining linear recurrence and attention outperform pure attention models in retrieval tasks.
...
Proof.
          ...
          St+1 = St
           ...
           αt+1(I − βt+1kt+1kt [⊺] +1 [)] + βt+1vt+1kt [⊺] +1
