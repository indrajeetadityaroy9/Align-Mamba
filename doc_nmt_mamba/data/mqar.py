...
class MQARConfig:
    """
    MQAR task config. d_state=64 forces state capacity cliff.
    Structure: [BOS, key1, :, val1, key2, :, val2, ..., QUERY, k1, k2, ..., EOS]
    Queries are a subset of keys; model must output corresponding values.
    """
    d_state: int = 64
    num_pairs: int = 64
    num_queries: int = 16
    ...
    eos_token_id: int = 2
    sep_token_id: int = 3
    kv_sep_token_id: int = 3
    query_token_id: int = 4

    key_token_start: int = 10
    key_token_end: int = 4096
    value_token_start: int = 4096
    value_token_end: int = 8192
...
class MQARDataset(Dataset):
    ...
    def _generate_sample(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.mode == "seq2seq":
            ...
            # Labels aligned for teacher forcing: position i predicts value for query i
            ...
            if len(src_ids) < max_src_len:
                src_ids.extend([self.config.pad_token_id] * (max_src_len - len(src_ids)))

            max_tgt_len = 1 + self.config.num_queries + 1
            if len(tgt_ids) < max_tgt_len:
                tgt_ids.extend([self.config.pad_token_id] * (max_tgt_len - len(tgt_ids)))
...
class MQARCurriculumGenerator:
    """Curriculum: starts easy (few pairs), increases to test state capacity limits."""
    ...
    def __init__(
        ...
        self.num_pairs_range = num_pairs_range or [16, 32, 64, 128, 256, 512]
    ...
    def generate_full_curriculum(self, seq_length: int = 512) -> Dict[int, MQARDataset]:
        stages = {}
    ...
    def generate_state_capacity_sweep(self, num_samples: int = 1000) -> List[Tuple[int, int, MQARDataset]]:
        """Sweep across num_pairs and seq_lengths. Skips configs where num_pairs * 4 > seq_length."""
        sweep = []
